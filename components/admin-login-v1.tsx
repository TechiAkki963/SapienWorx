...
export function AdminLoginV1() {
  ...
  const request = async () => {
    ...
    if (Object.keys(nextErrors).length) { setError("Check the highlighted email before continuing."); return; }
    try {
      setWorking(true); setError("");
      const response = await apiClient<{ transactionId: string }>("/api/auth/request-otp", { method: "POST", body: JSON.stringify({ flow: "SIGN_IN", role: "SUPER_ADMIN", email: email.trim() }) });
      setTransaction(response.transactionId); setDigits(["", "", "", "", "", ""]); window.setTimeout(() => digitRefs.current[0]?.focus(), 0);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Secure verification could not be started."); }
    finally { setWorking(false); }
  };

  const setDigit = (index: number, value: string) => { const digit = value.replace(/\D/g, "").slice(-1); setDigits((current) => current.map((item, itemIndex) => itemIndex === index ? digit : item)); if (digit && index < 5) digitRefs.current[index + 1]?.focus(); };
  const pasteDigits = (value: string) => { const next = value.replace(/\D/g, "").slice(0, 6).split(""); if (!next.length) return; setDigits(Array.from({ length: 6 }, (_, index) => next[index] ?? "")); digitRefs.current[Math.min(next.length, 6) - 1]?.focus(); };
  const verify = async () => {
    try {
      setWorking(true); setError("");
      const response = await apiClient<{ authenticated: boolean; redirectTo: string | null }>("/api/auth/verify-otp", { method: "POST", body: JSON.stringify({ transactionId: transaction, channel: "EMAIL", code }) });
