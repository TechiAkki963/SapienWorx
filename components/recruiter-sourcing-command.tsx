 ...
 return <WorkspaceShell workspace="recruiter" active="sourcing" title="Search talent" description="Persistent filters keep the sourcing context visible while you build a precise, privacy-safe candidate search.">
  <div className={styles.layout}>
   <aside className={styles.rail}>
    ...
    <section><h3>Keywords</h3><label><span>Preferred keywords</span><textarea rows={2} value={search.anyKeywords} onChange={e=>update("anyKeywords",e.target.value)} placeholder="Java, Spring Boot, Backend"/><small>These improve result order without excluding otherwise qualified candidates.</small></label><label><span>Must include</span><input value={search.allKeywords} onChange={e=>update("allKeywords",e.target.value)} placeholder="PostgreSQL, AWS"/></label><label><span>Exclude</span><input value={search.excludeKeywords} onChange={e=>update("excludeKeywords",e.target.value)} placeholder="Intern, fresher"/></label></section>
