...
import { AdminOrganisationBrandReview } from "./organisation-brand-kit";
...
export function MasterAdminConsole() {
  ...
  if (state) {
    ...
    if (tab === "governance") content = <><AdminOrganisationBrandReview/><Governance organisations={state.organisations.filter((item) => `${item.name} ${item.workEmailDomain}`.toLowerCase().includes(filter.toLowerCase()))} jobs={state.jobs.filter((item) => `${item.title} ${item.organisation} ${item.publicJobId} ${item.accountableRecruiter}`.toLowerCase().includes(filter.toLowerCase()))} onControl={controlSubject} onJob={(id, status, reason) => void mutate(`/api/admin/master/jobs/${id}`, { method: "PATCH", body: JSON.stringify({ status, reason }) }, `Job status changed to ${pretty(status)}.`)} /></>;
