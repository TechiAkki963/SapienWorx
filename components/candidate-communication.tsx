...
type Message = { sender: "them" | "me"; text: string; time: string };
type Conversation = { id: string; name: string; role: string; initial: string; tone: string; preview: string; time: string; unread?: boolean; messages: Message[] };
...
export function CandidateMessages() {
  ...
  const visibleConversations = useMemo(() => conversations.filter((conversation) => `${conversation.name} ${conversation.role} ${conversation.preview}`.toLowerCase().includes(query.trim().toLowerCase())), [query]);
  const messages = [...active.messages, ...(sent[active.id] ?? [])];
  const send = () => {
    if (!draft.trim()) return;
    setSent({ ...sent, [active.id]: [...(sent[active.id] ?? []), { sender: "me", text: draft.trim(), time: "Now" }] });
    setDraft("");
  };
  ...
  return <WorkspaceShell workspace="candidate" active="messages" title="Message centre" description="Keep all recruiter messages and InMail related to your applications in one place.">
    <section className="message-center" aria-label="Candidate message centre">
      <aside className="message-list">
        <header><div><span className="eyebrow">Messages & InMail</span><h2>Conversations</h2><p>{conversations.length} active conversations</p></div><span className="message-unread-count">{conversations.filter((conversation) => conversation.unread).length} unread</span></header>
        <label className="message-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations" aria-label="Search conversations" /></label>
        <div className="conversation-list">{visibleConversations.length > 0 ? visibleConversations.map((conversation) => <button type="button" aria-pressed={conversation.id === active.id} className={conversation.id === active.id ? "conversation active" : "conversation"} onClick={() => { setActiveId(conversation.id); setDraft(""); }} key={conversation.id}><span className={`conversation-avatar ${conversation.tone}`}>{conversation.initial}</span><div className="conversation-summary"><strong>{conversation.name}</strong><small>{conversation.role}</small><p>{conversation.preview}</p></div><time className="conversation-time">{conversation.unread ? <i aria-label="Unread" /> : null}{conversation.time}</time></button>) : <p className="message-empty">No conversations match that search.</p>}</div>
      </aside>
      <article className="conversation-panel">
        <header><div className="conversation-person"><span className={`conversation-avatar ${active.tone}`}>{active.initial}</span><div><strong>{active.name}</strong><small>{active.role}</small></div></div><Badge tone="green">Application open</Badge></header>
        <div className="message-context"><span>Product Designer</span><p>Northstar Labs · Applied 2 days ago</p></div>
        <div className="message-thread" aria-live="polite"><div className="message-day-divider"><span>Today</span></div>{messages.map((message, index) => <div className={`message-bubble ${message.sender}`} key={`${message.time}-${index}-${message.text}`}><p>{message.text}</p><small>{message.sender === "me" ? "You" : active.name} · {message.time}</small></div>)}</div>
        <footer className="message-composer"><label className="message-compose-field"><span className="sr-only">Message {active.name}</span><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={`Write a message to ${active.name.split(" ")[0]}…`} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} /></label><div><small>Enter to send · Shift + Enter for a new line</small><Button onClick={send} disabled={!draft.trim()}>Send message</Button></div></footer>
      </article>
    </section>
  </WorkspaceShell>;
}
