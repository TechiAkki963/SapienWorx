...
export function LoginPortal({ defaultPortal = "candidate", jobId, referralCode, shareSource }: { defaultPortal?: Portal; jobId?: string; referralCode?: string; shareSource?: string }) {
  // The route owns the role. Keeping it immutable prevents a candidate or recruiter
  // from changing portal context halfway through a security-sensitive sign-in flow.
  const portal: Portal = defaultPortal;
  ...
  return <AuthFrame eyebrow={portal === "candidate" ? "Candidate portal" : "Recruiter workspace"} title={title} copy={copy}>
    ...
    {step !== "reset-confirm" && step !== "reset-complete" && <p className="auth-footer">New to Sapienworx? <a href={portal === "candidate" ? sharedAuthPath("/register", jobId, referralCode, shareSource) : "/recruiter/register"}>Create an account</a><br/>{portal === "candidate" ? <>Hiring for a team? <a href="/recruiter/login">Recruiter sign in</a></> : <>Looking for a role? <a href={sharedAuthPath("/login", jobId, referralCode, shareSource)}>Candidate sign in</a></>}</p>}
  </AuthFrame>;
}
...
export function AuthFrame({ eyebrow, title, copy, children }: { eyebrow: string; title: string; copy: string; children: React.ReactNode }) {
  ...
  const recruiterJourney = /^Recruiter/i.test(eyebrow);
  const journey = recruiterJourney ? "recruiter" : "candidate";
  const aside = recruiterJourney
    ? {
        kicker: "Hiring workspace",
        title: <>Make every hiring decision<br/>with <em>context.</em></>,
        copy: "A focused workspace for thoughtful sourcing, confident outreach and a clear hiring record.",
        benefits: ["Verified work-email access", "Organisation-aware account setup", "Privacy-safe candidate data"],
      }
    : {
        kicker: "Candidate journey",
        title: <>Your next role deserves<br/>a <em>thoughtful start.</em></>,
        copy: "Build a profile that feels like you, find work worth doing and stay in control of every detail.",
        benefits: ["A profile you can review", "Secure contact verification", "Clear choices over your data"],
      };

  return <main className={`auth-page auth-page-${journey}`}><header className="auth-nav"><Logo/><div className="auth-nav-actions"><span>{recruiterJourney ? "For hiring teams" : "For your career"}</span><a href="/">Back to home</a></div></header><section className="auth-layout"><aside className="auth-aside"><span className="auth-aside-kicker">{aside.kicker}</span><h1>{aside.title}</h1><p>{aside.copy}</p><div className="auth-benefits">{aside.benefits.map((benefit) => <span key={benefit}>✓ {benefit}</span>)}</div><div className="auth-aside-note"><span>01</span><p>{recruiterJourney ? "A considered process from account setup to the first shortlist." : "A considered process from your first detail to your first opportunity."}</p></div></aside><section className="auth-card" aria-busy={!hydrated}><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="auth-copy">{copy}</p>{hydrated ? children : <div className="auth-hydration-state" role="status"><span aria-hidden="true"/>Preparing your secure form…</div>}</section></section></main>;
}
