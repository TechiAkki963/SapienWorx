...
export function LoginPortal({ defaultPortal = "candidate" }: { defaultPortal?: Portal }) {
  ...
  const requestSignIn = async () => {
    ...
    try {
      ...
      setStep("verify");
    } catch (error) { setNotice(error instanceof Error ? error.message : "We could not start secure sign-in."); }
  };
  ...
  return <AuthFrame eyebrow={portal === "candidate" ? "Candidate portal" : "Recruiter workspace"} title={step === "credentials" ? "Welcome back" : "Verify it’s you"} copy={step === "credentials" ? "Use the space built for your next career move." : "Enter both one-time codes before continuing."}>
    ...
    {step === "credentials" ? <form className="auth-form" onSubmit={(event) => { event.preventDefault(); void requestSignIn(); }}><AuthField label="Email address" type="email" value={contact.email} onChange={(value) => setContact({ ...contact, email: value })} placeholder={portal === "candidate" ? "you@example.com" : "you@company.com"}/><AuthField label="Password" type="password" value={password} onChange={setPassword} placeholder="Enter your password"/><div className="auth-row"><label><input type="checkbox"/> Remember this device</label><a href="#forgot">Forgot password?</a></div>{notice && <p className="consent-error" role="alert">{notice}</p>}<Button type="submit" disabled={!password}>Continue securely →</Button><p className="auth-microcopy">For every sign-in, we confirm both email and mobile ownership.</p></form> : <DualOtp contact={contact} methods={portal === "candidate" ? "both" : "email"} onVerify={verifySignIn}/>}
...
function CandidateRegistration() {
  ...
  async function startRegistration() {
    try {
      ...
      setTransactionId(response.transactionId);
      setStep("verify");
    } catch (error) { setNotice(error instanceof Error ? error.message : "We could not start registration."); }
  }
...
type RecruiterDetails = Contact & { location: string; organization: string; designation: string; password: string; confirmPassword: string };
...
function RecruiterRegistration() {
  ...
  const [details, setDetails] = useState<RecruiterDetails>({ name: "", email: "", mobile: "", location: "", organization: "", designation: "", password: "", confirmPassword: "" });
  const [consent, setConsent] = useState(false);
  const [error, setError] = useState("");
  const [transactionId, setTransactionId] = useState("");
  const emailOnly = recruiterType === "employer";
  const completeDetails = Boolean(details.name && details.location && details.mobile && details.email && details.organization && details.designation && details.password && details.confirmPassword && consent);
  ...
  const continueToVerification = async () => {
    ...
    try {
      const response = await apiClient<OtpRequestResponse>("/api/auth/request-otp", { method: "POST", body: JSON.stringify({ flow: recruiterType === "consultant" ? "CONSULTANT_REGISTRATION" : "RECRUITER_REGISTRATION", fullName: details.name, email: details.email, mobile: details.mobile, password: details.password, organisationName: details.organization, designation: details.designation, location: details.location }) });
      setTransactionId(response.transactionId);
      setStep("verify");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not start secure registration."); }
  };
  const verifyRegistration = async (codes: { emailCode: string; mobileCode: string }) => {
  ...
  return <AuthFrame eyebrow="Recruiter & consultant onboarding" title={step === "details" ? "Create your free account" : step === "verify" ? verificationLabel : "You’re securely verified"} copy={step === "details" ? "Post your first job in less than two minutes. Select the account type that best describes your hiring work." : step === "verify" ? emailOnly ? "A one-time code was sent to your official work email. Verify it to activate your recruiter workspace." : "One-time codes were sent to your official work email and mobile number. Verify both to access your consultant workspace." : emailOnly ? "Your official work email has been verified. Your recruiter workspace is ready." : "Your email and mobile have both been verified. Your consultant workspace is ready."}>
    {step === "details" ? <form className="auth-form recruiter-onboarding-form" onSubmit={(event) => { event.preventDefault(); void continueToVerification(); }}>
      ...
      <div className="recruiter-field-grid"><AuthField label="Name" value={details.name} onChange={(value) => update("name", value)} placeholder="Write your full name"/><SelectField label="Location" value={details.location} onChange={(value) => update("location", value)} placeholder="Select location" options={["Bengaluru", "Delhi NCR", "Hyderabad", "Mumbai", "Pune", "Remote"]}/><AuthField label="Phone" type="tel" value={details.mobile} onChange={(value) => update("mobile", value)} placeholder="e.g. 9873721034"/><AuthField label="Work email id" type="email" value={details.email} onChange={(value) => update("email", value)} placeholder="Your official email id (not gmail/yahoo)"/><SelectField label="Organization" value={details.organization} onChange={(value) => update("organization", value)} placeholder="Select from dropdown" options={["Nexora Technologies", "Northstar Labs", "Cascade", "Plume Health", "Kinetic", "Other organization"]}/><AuthField label="Designation" value={details.designation} onChange={(value) => update("designation", value)} placeholder="e.g. Talent Acquisition Manager"/><AuthField label="Password" type="password" value={details.password} onChange={(value) => update("password", value)} placeholder="Set a password"/><AuthField label="Confirm password" type="password" value={details.confirmPassword} onChange={(value) => update("confirmPassword", value)} placeholder="Confirm password"/></div>
    ...
    </form> : step === "verify" ? <DualOtp contact={details} methods={emailOnly ? "email" : "both"} onVerify={verifyRegistration}/> : <div className="verified-state"><span>✓</span><h2>Account verified</h2><p>{emailOnly ? "Your official email is verified. You can now set up your organization and publish your first role." : "Your official email and mobile are verified. You can now access your consultant workspace."}</p><Button href="/recruiter">Open recruiter workspace →</Button></div>}
...
function SelectField({ label, placeholder, value, onChange, options }: { label: string; placeholder: string; value: string; onChange: (value: string) => void; options: string[] }) { return <label className="auth-field"><span>{label}</span><select required value={value} onChange={(event) => onChange(event.target.value)}><option value="" disabled>{placeholder}</option>{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
...
function DualOtp({ contact, onVerified, onVerify, methods = "both" }: { contact: Contact; onVerified?: () => void; onVerify?: (codes: { emailCode: string; mobileCode: string }) => Promise<void>; methods?: "email" | "both" }) {
  const [emailCode, setEmailCode] = useState("");
  const [mobileCode, setMobileCode] = useState("");
  const [error, setError] = useState("");
  const [verifying, setVerifying] = useState(false);
  const ready = emailCode.length === 6 && (methods === "email" || mobileCode.length === 6);
  const verify = async () => { setError(""); setVerifying(true); try { if (onVerify) await onVerify({ emailCode, mobileCode }); else onVerified?.(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not verify these codes."); } finally { setVerifying(false); } };
  return <div className="dual-otp"><div className="otp-method"><div><strong>Email code</strong><small>Sent to {maskEmail(contact.email)}</small></div><OtpInputs label="Email verification code" onChange={setEmailCode}/></div>{methods === "both" && <div className="otp-method"><div><strong>Mobile code</strong><small>Sent to {maskMobile(contact.mobile)}</small></div><OtpInputs label="Mobile verification code" onChange={setMobileCode}/></div>}{error && <p className="consent-error" role="alert">{error}</p>}<Button onClick={() => { void verify(); }} disabled={!ready || verifying}>{verifying ? "Verifying…" : methods === "email" ? "Verify email code" : "Verify both codes"}</Button><p className="auth-microcopy">Digits advance automatically as you type. Codes expire after 10 minutes and are never stored as plain text.</p></div>;
}
...
function OtpInputs({ label, onChange }: { label: string; onChange: (value: string) => void }) { const [digits, setDigits] = useState<string[]>(Array(6).fill("")); const inputRefs = useRef<Array<HTMLInputElement | null>>([]); const updateDigits = (next: string[]) => { setDigits(next); onChange(next.join("")); }; const fillDigits = (start: number, value: string) => { const incoming = value.replace(/\D/g, "").slice(0, 6 - start); const next = [...digits]; if (!incoming) { next[start] = ""; updateDigits(next); return; } incoming.split("").forEach((digit, offset) => { next[start + offset] = digit; }); updateDigits(next); const nextIndex = Math.min(start + incoming.length, 5); requestAnimationFrame(() => inputRefs.current[nextIndex]?.focus()); }; return <div className="otp-inputs" aria-label={label}>{digits.map((digit, index) => <input key={index} ref={(element) => { inputRefs.current[index] = element; }} inputMode="numeric" maxLength={1} value={digit} aria-label={`${label} digit ${index + 1}`} onChange={(event) => fillDigits(index, event.target.value)} onPaste={(event) => { event.preventDefault(); fillDigits(index, event.clipboardData.getData("text")); }} onKeyDown={(event) => { if (event.key === "Backspace" && !digits[index] && index > 0) requestAnimationFrame(() => inputRefs.current[index - 1]?.focus()); }} />)}</div>; }
