...
export function CandidateJobs({ sharedJobId, sharedApplyOutcome }: { sharedJobId?: string; sharedApplyOutcome?: string } = {}) {
  ...
  const visibleJobs = useMemo(() => [...matchingJobs]
    ...
    .sort((left, right) => sort === "RECENT" ? left.postedDays - right.postedDays : sort === "SALARY" ? (right.maximumSalary ?? -1) - (left.maximumSalary ?? -1) : right.matchScore - left.matchScore), [matchingJobs, saved, sort, view]);
