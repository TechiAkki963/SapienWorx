...
type CandidateJob = { id: string; company: string; role: string; department: string; skills: string[]; location: string; type: string; minimumExperience: number; maximumExperience: number; minimumSalary: number | null; maximumSalary: number | null; postedDays: number; logo: string; color: string; description: string; publicPath: string };
...
export function CandidateJobs({ sharedJobId, sharedApplyOutcome }: { sharedJobId?: string; sharedApplyOutcome?: string } = {}) {
  ...
  useEffect(() => {
    ...
    void apiClient<PublicJobPage>("/api/public/jobs", { signal: controller.signal }).then((response) => {
    ...
    }).finally(() => { if (!controller.signal.aborted) setLoadingJobs(false); });
  ...
  const visibleJobs = useMemo(() => [...matchingJobs]
    .filter((job) => view === "MATCHES" || saved.includes(job.id))
    .sort((left, right) => sort === "RECENT" ? left.postedDays - right.postedDays : sort === "SALARY" ? (right.maximumSalary ?? -1) - (left.maximumSalary ?? -1) : 0), [matchingJobs, saved, sort, view]);
  const locationOptions = useMemo(() => Array.from(new Set(jobs.map((job) => job.location.split(" · ")[0]))).sort(), [jobs]);
  const companyOptions = useMemo(() => Array.from(new Set(jobs.map((job) => job.company))).sort(), [jobs]);
