...
function JobShareModal({ job, onClose }: { job: ShareableJob; onClose: () => void }) {
  ...
  const facebookUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(sourceUrl("facebook"))}`;
  ...
  return <div className="job-share-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="job-share-modal" role="dialog" aria-modal="true" aria-labelledby="job-share-title"><button className="job-share-close" onClick={onClose} aria-label="Close share dialog">×</button><span className="eyebrow">Job published</span><h2 id="job-share-title">Share {job.title}</h2><p>LinkedIn, X, and Facebook will use this role-specific branded preview. Every application is delivered to the recruiter who posted the vacancy.</p><article className="job-share-card" aria-label="Social media card preview"><div className="job-share-brand"><Image src="/brand/sapienworx-mark.jpeg" alt="Sapienworx" width={122} height={121} sizes="(max-width: 600px) 88px, 122px"/><b>Sapienworx</b></div><div className="job-share-divider" aria-hidden="true"/><div className="job-share-role"><div className="job-share-company"><span>{candidateInitials(job.company)}</span><b>{job.company}</b></div><h3>{job.title}</h3><p>{job.experience.replace("Yrs Exp", "years")}</p><p>{job.location}</p><footer>www.sapienworx.com</footer></div></article><label className="job-share-url"><span>Public job URL</span><div><code>{publicUrl}</code><button type="button" onClick={() => { void copyPublicUrl(); }}>Copy</button></div></label><div className="job-share-actions"><a href={linkedInUrl} target="_blank" rel="noopener noreferrer" onClick={() => setSharedWith("Opening LinkedIn share")}>in LinkedIn</a><a href={xUrl} target="_blank" rel="noopener noreferrer" onClick={() => setSharedWith("Opening X share")}>𝕏 X</a><a href={facebookUrl} target="_blank" rel="noopener noreferrer" onClick={() => setSharedWith("Opening Facebook share")}>f Facebook</a><a href={whatsAppUrl} target="_blank" rel="noopener noreferrer" onClick={() => setSharedWith("Opening WhatsApp share")}>◉ WhatsApp</a></div><small className="job-share-routing-note">✓ Applications from every valid shared link stay attached to {job.company}&apos;s posting recruiter.</small>{sharedWith && <small className="positive" role="status">{sharedWith}</small>}</section></div>;
}
