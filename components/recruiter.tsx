...
type ShareableJob = { jobId: string; title: string; location: string; skills: string[]; experience: string };
...
function RecruiterJobsContent() {
  ...
  const publish = async () => { if (!readyToPublish) return; const publishedJobId = await ensureJobId(); if (!publishedJobId) return; setStatus("published"); setShareJob({ jobId: publishedJobId, title, location, skills, experience: experienceLabel }); };
...
export function PublicJobDetail({ jobId, slug, fromSearch = false, initialJob }: { jobId: string; slug: string; fromSearch?: boolean; initialJob?: PublicJobDetailData | null }) {
  ...
  return <main className="public-page public-job-detail-page"><header className="public-nav"><Logo /><nav aria-label="Public navigation"><a href="/jobs">Find jobs</a><a href="/companies">Companies</a><a href="/knowledge">Knowledge hub</a></nav><div className="public-nav-actions"><a className="text-action" href="/login">Sign in</a><a className="button button-primary" href={`/register?job=${encodeURIComponent(job.jobId)}`}>Create profile</a></div></header><nav className="public-job-breadcrumb public-container" aria-label="Breadcrumb"><a href="/">Home</a><span>/</span><a href="/jobs">Jobs</a><span>/</span><strong>{job.title}</strong></nav>{fromSearch && <div className="public-container"><a className="public-job-search-back" href="/jobs">‹ Back to search results</a></div>}<section className="public-job-detail-hero"><div className="public-container"><div className="public-job-detail-brand"><span>{company.slice(0, 1)}</span><p>{company}</p></div><div className="public-job-detail-heading"><div><span className="eyebrow">Sapienworx verified role</span><h1>{job.title}</h1><div className="public-job-detail-meta"><span>{job.minimumExperience}–{job.maximumExperience} years</span>{salary}<span>{job.location}</span></div></div><a className="button button-primary public-job-hero-apply" href={`/register?job=${encodeURIComponent(job.jobId)}`}>Apply now <span>→</span></a></div></div></section><main className="public-job-detail-layout public-section"><article className="public-job-description"><span className="eyebrow">About the role</span><h2>Build your next chapter with {company}.</h2><div className="public-job-skill-list">{job.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="rich-job-render" dangerouslySetInnerHTML={{ __html: job.description }}/></article><aside className="public-job-apply-panel"><strong>{job.title}</strong><p>{job.location}</p><a className="button button-primary" href={`/register?job=${encodeURIComponent(job.jobId)}`}>Apply now <span>→</span></a><small>Applying takes you to a secure Sapienworx candidate profile flow.</small><a className="public-job-copy-link" href={canonicalPath}>Open canonical link</a></aside></main><div className="public-job-mobile-apply"><a className="button button-primary" href={`/register?job=${encodeURIComponent(job.jobId)}`}>Apply now <span>→</span></a></div></main>;
}
...
export function RecruiterJobList({ initialFilters = {} }: { initialFilters?: JobManagementInitialFilters }) {
  ...
  const toShareableJob = (job: ManagedJob): ShareableJob => ({ jobId: job.jobId, title: job.title, location: job.location, skills: job.skills, experience: `${job.minimumExperience}–${job.maximumExperience} years` });
...
function JobShareModal({ job, onClose }: { job: ShareableJob; onClose: () => void }) {
  ...
  const copyPublicUrl = async () => { try { await navigator.clipboard.writeText(new URL(publicUrl, window.location.origin).toString()); setSharedWith("Public link copied"); } catch { setSharedWith("Public link ready to copy"); } };
  return <div className="job-share-modal-backdrop" role="presentation"><section className="job-share-modal" role="dialog" aria-modal="true" aria-labelledby="job-share-title"><button className="job-share-close" onClick={onClose} aria-label="Close share dialog">×</button><span className="eyebrow">Job published</span><h2 id="job-share-title">Share {job.title}</h2><p>Distribute this Sapienworx role with a branded card and public application link.</p><article className="job-share-card"><header><span>S</span><b>Sapienworx</b><small>{job.jobId}</small></header><div><small>Nexora Technologies is hiring</small><h3>{job.title}</h3><p>{job.experience} · {job.skills.slice(0, 3).join(" · ")}</p><footer>{job.location}</footer></div></article><label className="job-share-url"><span>Public job URL</span><div><code>{publicUrl}</code><button onClick={() => { void copyPublicUrl(); }}>Copy</button></div></label><div className="job-share-actions"><button onClick={() => setSharedWith("LinkedIn share ready")}>in LinkedIn</button><button onClick={() => setSharedWith("Twitter share ready")}>𝕏 Twitter</button><button onClick={() => setSharedWith("WhatsApp share ready")}>◉ WhatsApp</button></div>{sharedWith && <small className="positive" role="status">{sharedWith}</small>}</section></div>;
}
...
function RecruiterPipelineContent() {
  ...
  const matchingCandidates = candidates.filter((candidate) => {
    const attributes = candidateAttributes[candidate.id];
  ...
  const moveCandidate = (id: number, stage: Candidate["stage"]) => setCandidates((current) => current.map((candidate) => candidate.id === id ? { ...candidate, stage, profileUpdated: "Just now" } : candidate));
  const updateNote = (id: number, note: string) => setCandidates((current) => current.map((candidate) => candidate.id === id ? { ...candidate, note } : candidate));
...
function PipelineListRow({ candidate, emailRevealed, phoneRevealed, emailPending, phonePending, noteExpanded, actionNotice, onEmailAccess, onPhoneAccess, onToggleNote, onStageChange, onNoteChange, onInMail }: { candidate: Candidate; emailRevealed: boolean; phoneRevealed: boolean; emailPending: boolean; phonePending: boolean; noteExpanded: boolean; actionNotice?: string; onEmailAccess: () => void; onPhoneAccess: () => void; onToggleNote: () => void; onStageChange: (stage: Candidate["stage"]) => void; onNoteChange: (note: string) => void; onInMail: () => void }) {
  return <article className="pipeline-list-row"><div className="pipeline-list-identity"><span className="pipeline-list-avatar">{candidate.initials}</span><div className="pipeline-list-person"><div><h2>{candidate.name}</h2><Badge tone={candidate.score > 92 ? "green" : "blue"}>{candidate.score}% match</Badge></div><p>{candidate.role}</p></div><div className="pipeline-inline-note"><button onClick={onToggleNote} aria-expanded={noteExpanded}><span>▤</span>{noteExpanded ? "Close recruiter note" : "View / add recruiter note"}</button>{noteExpanded && <label><span>Recruiter note for {candidate.name}</span><textarea value={candidate.note} onChange={(event) => onNoteChange(event.target.value)} placeholder="Add a screening note…"/></label>}</div></div><div className="pipeline-contact"><ContactPill type="email" value={candidate.email} revealed={emailRevealed} pending={emailPending} onClick={onEmailAccess}/><ContactPill type="phone" value={candidate.phone} revealed={phoneRevealed} pending={phonePending} onClick={onPhoneAccess}/></div><label className="pipeline-stage-select"><span>Pipeline stage</span><select aria-label={`Move ${candidate.name} to`} value={candidate.stage} onChange={(event) => onStageChange(event.target.value as Candidate["stage"])}>{stages.map((stage) => <option value={stage} key={stage}>{stage}</option>)}</select></label><div className="pipeline-row-actions"><a href={`/recruiter/communications?candidate=${encodeURIComponent(candidate.name)}`}>Message</a><button onClick={onInMail}>InMail</button>{actionNotice && <small role="status">{actionNotice}</small>}</div><footer className="pipeline-row-micro-details"><span>Profile last updated {candidate.profileUpdated}</span><span>Last active {candidate.lastActive}</span></footer></article>;
}
