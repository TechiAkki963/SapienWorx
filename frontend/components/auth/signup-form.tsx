  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiRequest<{ email: string; development_otp?: string }>(
        recruiter ? "/api/v1/auth/recruiter/register" : "/api/v1/auth/candidate/register",
  ...
  return (
    <form className="grid gap-4" onSubmit={submit}>
      ...
      <fieldset className="grid gap-3 rounded-2xl border border-line bg-slate-50/70 p-4">
        <legend className="px-1 text-sm font-bold text-navy">Privacy acknowledgement</legend>
        <p className="text-xs leading-5 text-ink-muted">SapienWorx uses account and recruitment information to provide the service, secure accounts, support hiring workflows and meet applicable obligations.</p>
        <label className="flex items-start gap-3 text-sm leading-5 text-ink">
          <input name="privacy_consent" type="checkbox" required className="mt-1 h-4 w-4 rounded border-line accent-indigo" />
          <span>I have reviewed the privacy information for account and recruitment data processing.</span>
        </label>
        {!recruiter && <label className="flex items-start gap-3 text-sm leading-5 text-ink"><input name="age_confirmed" type="checkbox" required className="mt-1 h-4 w-4 rounded border-line accent-indigo" /><span>I confirm that I am 18 years of age or older.</span></label>}
      </fieldset>
