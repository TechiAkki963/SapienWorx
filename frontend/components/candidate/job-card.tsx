...
export function JobCard({ job, compact = false }: { job: CandidateJob; compact?: boolean }) {
    ...
    <Surface className="group relative h-full p-5 transition duration-200 hover:-translate-y-0.5 hover:border-indigo/15 hover:shadow-card">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-2 text-xl font-bold tracking-[-0.025em] text-ink"><Link className="outline-none after:absolute after:inset-0 focus-visible:underline" href={`/jobs/${job.id}`}>{job.title}</Link></h3>
        </div>
