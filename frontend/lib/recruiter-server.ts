...
export async function recruiterAPI<T>(path:string):Promise<T>{const store=await cookies();const response=await fetch(`${SERVER_API_URL}${path}`,{headers:{cookie:store.toString()},cache:"no-store"});if(!response.ok)throw new RecruiterBackendError(await parse(response),response.status);return await response.json() as T;}
