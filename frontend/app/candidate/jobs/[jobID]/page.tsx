...
export default async function CandidateJobDetailPage({ params }: Props) {
  ...
  const initialSaved = saved.items.some((item) => item.id === job.id);
  const initialApplied = applications.items.some((item) => item.job_id === job.id);
  const salary = salaryLabel(job);

  return (
    <div>
      <Button href="/candidate/jobs" variant="ghost" size="sm">← Back to jobs</Button>
      <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-start">
        <article className="min-w-0">
          <Surface className="p-6 sm:p-8">
            ...
            <h1 className="mt-3 text-balance text-4xl font-bold leading-tight tracking-[-0.045em] text-navy sm:text-5xl">{job.title}</h1>
        ...
        <aside className="lg:sticky lg:top-24"><Surface className="p-5" tone="mint"><p className="text-xs font-bold uppercase tracking-[0.14em] text-indigo">Your next step</p><h2 className="mt-2 text-xl font-bold">Interested in this role?</h2><p className="mt-2 text-sm leading-6 text-ink-muted">Apply here and the role will stay connected to your candidate tracker.</p><div className="mt-5"><JobActions jobId={job.id} isCandidate initialSaved={initialSaved} initialApplied={initialApplied} /></div></Surface></aside>
      </div>
    </div>
  );
}
