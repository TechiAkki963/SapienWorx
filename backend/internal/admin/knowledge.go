...
func validateKnowledge(in KnowledgeInput) (KnowledgeInput, error) {
 ...
 if in.AuthorName == "" { in.AuthorName = "SapienWorx Editorial" }
 if in.Status != "draft" && in.Status != "published" { return in, ErrInvalid }
 if !knowledgeSlug.MatchString(in.Slug) || utf8.RuneCountInString(in.Slug) > 100 ||
    ...
    in.FeaturedOrder < 0 || in.FeaturedOrder > 1000 { return in, ErrInvalid }
 return in, nil
}
...
func (s *Service) PublishedKnowledge(ctx context.Context, category string) (KnowledgeList,error) {
 ...
 if category!="" && !knowledgeCategories[category] {return KnowledgeList{},ErrInvalid}
 ...
 if err!=nil{return KnowledgeList{},err}
 defer rows.Close()
...
func (s *Service) PublishedKnowledgeArticle(ctx context.Context, slug string) (KnowledgeArticle,error) {
 if !knowledgeSlug.MatchString(slug) {return KnowledgeArticle{},ErrNotFound}
 ...
 if errors.Is(err,pgx.ErrNoRows){return KnowledgeArticle{},ErrNotFound}
 return a,err
}
...
func (s *Service) CreateKnowledge(ctx context.Context,in KnowledgeInput,adminID,ip,requestID string)(KnowledgeArticle,error){
 in,err:=validateKnowledge(in);if err!=nil{return KnowledgeArticle{},err}
 tx,err:=s.db.BeginTx(ctx,pgx.TxOptions{});if err!=nil{return KnowledgeArticle{},err}
 ...
 if err!=nil{return KnowledgeArticle{},err}
 id:=a.ID
 ...
 if err!=nil{return KnowledgeArticle{},err}
 if err=tx.Commit(ctx);err!=nil{return KnowledgeArticle{},err}
 return a,nil
}
...
func (s *Service) UpdateKnowledge(ctx context.Context,id string,in KnowledgeInput,adminID,ip,requestID string)(KnowledgeArticle,error){
 in,err:=validateKnowledge(in);if err!=nil{return KnowledgeArticle{},err}
 if in.Revision<1 {return KnowledgeArticle{},ErrInvalid}
 tx,err:=s.db.BeginTx(ctx,pgx.TxOptions{});if err!=nil{return KnowledgeArticle{},err}
 defer tx.Rollback(ctx)
 var oldRevision int
 ...
 if errors.Is(err,pgx.ErrNoRows){return KnowledgeArticle{},ErrNotFound}
 if err!=nil{return KnowledgeArticle{},err}
 if oldRevision!=in.Revision{return KnowledgeArticle{},ErrKnowledgeConflict}
