...
func validateOfficialEmail(email string) error {
	domain := emailDomain(email)
	if domain == "" {
		return errors.New("valid work email is required")
	}
	if _, blocked := publicEmailDomains[domain]; blocked {
		return errors.New("recruiters must use an official company email address")
	}
	return nil
}
func validatePassword(password string) error {
	if len(password) < 12 || len([]byte(password)) > 72 {
		return errors.New("password must be at least 12 characters and no more than 72 bytes")
	}
	return nil
}
func validatePhone(phone string) error {
	if !phonePattern.MatchString(strings.TrimSpace(phone)) {
		return errors.New("phone must use E.164 format, for example +919876543210")
	}
	return nil
}
