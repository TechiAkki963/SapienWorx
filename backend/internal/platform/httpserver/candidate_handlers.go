...
func (s *Server) listJobs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { s.writeCandidateError(w, r, err); return }
...
func (s *Server) candidateDashboard(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	result, err := s.candidate.Dashboard(r.Context(), id); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusOK, result)
}
...
func (s *Server) candidateProfile(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		result, err := s.candidate.Profile(r.Context(), id); if err != nil { s.writeCandidateError(w, r, err); return }
		writeJSON(w, http.StatusOK, result); return
	}
	var input candidate.ProfileUpdate
	if !decodeJSON(w, r, &input) { return }
	result, err := s.candidate.UpdateProfile(r.Context(), id, input); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusOK, result)
}
...
func (s *Server) candidateApplications(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	if r.Method == http.MethodGet {
		items, err := s.candidate.Applications(r.Context(), id, 50); if err != nil { s.writeCandidateError(w, r, err); return }
		writeJSON(w, http.StatusOK, map[string]any{"items": items}); return
	}
	var input struct { JobID string `json:"job_id"` }
	if !decodeJSON(w, r, &input) { return }
	if strings.TrimSpace(input.JobID) == "" { writeError(w, r, http.StatusBadRequest, "validation_error", "job_id is required"); return }
	item, err := s.candidate.Apply(r.Context(), id, input.JobID); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusCreated, item)
}

func (s *Server) candidateApplicationWithdraw(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	if err := s.candidate.WithdrawApplication(r.Context(), id, r.PathValue("applicationID")); err != nil { s.writeCandidateError(w, r, err); return }
	w.WriteHeader(http.StatusNoContent)
}

func (s *Server) candidateInterviews(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	items, err := s.candidate.InterviewsForCandidate(r.Context(), id); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusOK, map[string]any{"items": items})
}

func (s *Server) candidateSavedJobs(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	items, err := s.candidate.SavedJobs(r.Context(), id); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusOK, map[string]any{"items": items})
}
...
func (s *Server) candidateSavedJob(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if r.Method == http.MethodPut { err = s.candidate.SaveJob(r.Context(), id, jobID) } else { err = s.candidate.UnsaveJob(r.Context(), id, jobID) }
	if err != nil { s.writeCandidateError(w, r, err); return }
	w.WriteHeader(http.StatusNoContent)
}
...
func (s *Server) candidateNotifications(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	items, err := s.candidate.Notifications(r.Context(), id, 50); if err != nil { s.writeCandidateError(w, r, err); return }
	writeJSON(w, http.StatusOK, map[string]any{"items": items})
}

func (s *Server) candidateNotificationRead(w http.ResponseWriter, r *http.Request) {
	id, ok := candidateID(r); if !ok { writeError(w, r, http.StatusUnauthorized, "unauthorized", "authentication required"); return }
	if err := s.candidate.MarkNotificationRead(r.Context(), id, r.PathValue("notificationID")); err != nil { s.writeCandidateError(w, r, err); return }
	w.WriteHeader(http.StatusNoContent)
}
...
func (s *Server) writeCandidateError(w http.ResponseWriter, r *http.Request, err error) {
	...
	default:
		if strings.Contains(err.Error(), "required") || strings.Contains(err.Error(), "negative") { writeError(w, r, http.StatusBadRequest, "validation_error", err.Error()); return }
