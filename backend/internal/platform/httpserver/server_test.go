...
func testConfig() config.Config {
	return config.Config{
		...
		Auth:     config.AuthConfig{AccessCookieName: "sw_access", RefreshCookieName: "sw_refresh"},
	}
}
...
func TestReadinessFailsWhenDatabaseFails(t *testing.T) {
	server := New(testConfig(), fakeDB{err: errors.New("down")}, testTokens(t), nil, nil, slog.Default())
...
func TestProtectedEndpointRejectsMissingToken(t *testing.T) {
	server := New(testConfig(), fakeDB{}, testTokens(t), nil, nil, slog.Default())
