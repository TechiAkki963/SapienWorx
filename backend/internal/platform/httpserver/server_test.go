...
func TestReadinessFailsWhenDatabaseFails(t *testing.T) {
	server := New(testConfig(), fakeDB{err: errors.New("down")}, testTokens(t), nil, nil, nil, slog.Default())
...
func TestProtectedEndpointRejectsMissingToken(t *testing.T) {
	server := New(testConfig(), fakeDB{}, testTokens(t), nil, nil, nil, slog.Default())
