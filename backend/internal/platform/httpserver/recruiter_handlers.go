...
func (s *Server) recruiterJobStatus(w http.ResponseWriter, r *http.Request) {
	...
	if !decodeJSON(w, r, &input) { return }
	if err := s.recruiter.SetJobStatus(r.Context(), id, r.PathValue("jobID"), strings.TrimSpace(input.Status)); err != nil { s.writeRecruiterError(w, r, err); return }
...
func (s *Server) recruiterJobCompensation(w http.ResponseWriter, r *http.Request) {
	...
	if !decodeJSON(w, r, &input) { return }
	if err := s.recruiter.SetJobCompensation(r.Context(), id, r.PathValue("jobID"), input); err != nil { s.writeRecruiterError(w, r, err); return }
	w.WriteHeader(http.StatusNoContent)
}
...
func (s *Server) recruiterApplicationStage(w http.ResponseWriter, r *http.Request) {
	id, ok := recruiterID(r)
	if !ok { return }
	var input struct { Stage string `json:"stage"` }
	if !decodeJSON(w, r, &input) { return }
	if err := s.recruiter.UpdateStage(r.Context(), id, r.PathValue("applicationID"), strings.TrimSpace(input.Stage)); err != nil { s.writeRecruiterError(w, r, err); return }
	w.WriteHeader(http.StatusNoContent)
}
