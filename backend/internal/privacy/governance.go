...
func (s *Service) PublicSubprocessors(ctx context.Context) ([]Subprocessor, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Purpose, &item.DataCategories, &item.ProcessingLocations, &item.WebsiteURL, &item.DPAURL, &item.EffectiveFrom); err != nil { return nil, err }
...
func (s *Service) AdminRequests(ctx context.Context) ([]AdminPrivacyRequest, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.UserID, &item.RequestType, &item.Status, &item.DueAt, &item.CompletedAt, &item.CreatedAt); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (s *Service) Incidents(ctx context.Context) ([]PrivacyIncident, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Title, &item.Severity, &item.Status, &item.DiscoveredAt, &item.ContainedAt, &item.AffectedSubjectsEstimate, &item.NotificationRequired, &item.NotificationDeadline); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (s *Service) ProcessingActivities(ctx context.Context) ([]ProcessingActivity, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.ActivityName, &item.Purpose, &item.LawfulBasis, &item.RetentionPolicy, &item.Owner, &item.ReviewedAt, &item.UpdatedAt); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
